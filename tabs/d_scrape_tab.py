...
class ScraperThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            all_results = []
            df = self.data_store.get_members_data()
            if df is None or len(df) == 0:
                self.scraping_error.emit("No member data available")
                return
            mswa_ids = df["MSWA number"].tolist()  # Convert to list
            total = len(mswa_ids)
            ...
            for idx, mswa_id in enumerate(mswa_ids):
                ...
                # Get swimmer name
                swimmer_name = self.data_store.get_member_name(mswa_id)
...
class ScrapeTab(QWidget):
    ...
    def start_scraping(self):
        ...
        self.scraper_thread = ScraperThread(self.data_store, year)
